...
(() => {
  ...
  const st = {
    d: { products: [], agents: [], customers: [], orders: [], customerOrders: [], gifts: [], returns: [], customerReturns: [], payments: [], inventoryMoves: [] },
    u: { p: "dashboard", pp: 1, ap: 1, cp: 1, items: [], cItems: [], editOrderId: "", editCustomerOrderId: "", f: { from: "", to: "", agentId: "all" } },
  ...
  const DB = {
    ...
    clear() {
      ...
      st.d = { products: [], agents: [], customers: [], orders: [], customerOrders: [], gifts: [], returns: [], customerReturns: [], payments: [], inventoryMoves: [] };
    }
  };
  ...
  const CO = {
    ...
    save() {
      ...
      if (target) {
        ...
        st.d.customerOrders.push({ id: orderId, customerId: cid, date, items, total });
      }
      DB.save();
      this.reset();
      R.all();
  ...
  const Gf = {
    save() {
      ...
      const target = (el.giftTarget.value || "").trim();
      ...
      if (+p.currentStock < bags) return UI.toast(`رصيد غير كاف: ${p.name}`, "error");

      const value = bags * pricePerBag(p);
      p.currentStock = +p.currentStock - bags;
      st.d.gifts.push({ id: U.id(), type, productId: pid, qty, unit, bagsQty: bags, value, target, note, date });
      st.d.inventoryMoves.push({ id: U.id(), type: "out", productId: pid, qty: bags, value, date, ref: type === "gift" ? "gift" : "sample" });
      DB.save();
      el.giftQty.value = "";
      el.giftTarget.value = "";
      el.giftNote.value = "";
      R.all();
      UI.toast("تم تسجيل الحركة");
    }
  };
  ...
  const O = {
    ...
    save() {
      ...
      if (target) {
        ...
        st.d.orders.push({ id: orderId, agentId: aid, date, items, total });
      }
      DB.save();
      this.reset();
      R.all();
        ...
        rows
          .map(o => {
            ...
            return `<tr>
              ...
              <td>${U.e(itemsText || "-")}</td>
              <td>${U.m(o.total)}</td>
              <td><button class='btn ghost' data-ore='${o.id}'>تعديل</button><button class='btn danger' data-ord='${o.id}'>حذف</button></td>
            </tr>`;
          })
        ...
        rows
          .map(o => {
            ...
            return `<tr>
              ...
              <td>${U.e(itemsText || "-")}</td>
              <td>${U.m(o.total)}</td>
              <td><button class='btn ghost' data-core='${o.id}'>تعديل</button><button class='btn danger' data-cord='${o.id}'>حذف</button></td>
            </tr>`;
          })
          .join("") || "<tr><td colspan='5'>لا توجد عمليات صرف</td></tr>";
    },
