...
(() => {
  ...
  const st = {
    d: { products: [], agents: [], orders: [], returns: [], payments: [], inventoryMoves: [] },
    u: { p: "dashboard", pp: 1, ap: 1, items: [], f: { from: "", to: "", agentId: "all" } },
  ...
  const DB = {
    ...
    clear() {
      ...
      st.d = { products: [], agents: [], orders: [], returns: [], payments: [], inventoryMoves: [] };
    }
  };
  ...
  const C = {
    ...
    stats() {
      ...
      const tv = st.d.orders.reduce((a, o) => a + +o.total, 0);
  ...
  const O = {
    ...
    save() {
      ...
      st.d.orders.push({ id: U.id(), agentId: aid, date, items, total });
      DB.save();
      this.reset();
      R.all();
      UI.toast("تم حفظ الطلبية");
    }
  };
  const Rt = {
    bind() {
  ...
  const Rp = {
    ...
    title(k) {
      const m = { agents: "تقرير المناديب", warehouse: "تقرير حركة المخزن", stock: "تقرير المخزون", topProducts: "تقرير أكثر المنتجات مبيعًا", profit: "تقرير إجمالي الأرباح" };
      return m[k] || "تقرير";
    },
    aRows() {
      let g = st.d.agents;
    ...
    wRows() {
      return C.fd(st.d.inventoryMoves).map(m => {
        const p = st.d.products.find(x => x.id === m.productId);
        const tp = m.type === "in" ? (m.ref === "opening" ? "أول المدة" : m.ref === "factory" ? "وارد مصنع" : "وارد") : m.type === "out" ? "منصرف" : "مرتجع";
    ...
    pRows() {
      const o = C.fd(st.d.orders).reduce((a, b) => a + +b.total, 0);
      const r = C.fd(st.d.returns).reduce((a, b) => a + +b.total, 0);
      return [
        { title: "إجمالي المبيعات", amount: o },
        { title: "إجمالي المرتجعات", amount: r },
        { title: "صافي الإيراد", amount: o - r }
      ];
    },
    render() {
      const a = this.aRows();
    ...
    print(k) {
      const m = { agents: el.agentsReportBody, warehouse: el.warehouseReportBody, stock: el.stockReportBody, topProducts: el.topProductsReportBody, profit: el.profitReportBody };
      const t = m[k];
      if (!t) return;
  ...
  const R = {
    stats() {
      ...
      const c = [
        ...
        { t: "إجمالي قيمة البضاعة المنصرفة", v: U.m(s.tv), m: "إجمالي حركة الصرف", ic: "💰" }
      ];
      ...
      el.inventoryTableBody.innerHTML =
        st.d.products
          .map(p => {
            ...
            const openingMovesHtml =
              Inv.productOpeningMoves(p.id)
                .slice()
                .sort((a, b) => String(b.date).localeCompare(String(a.date)))
                .slice(0, 5)
                .map(m => {
                  const delta = Inv.openingDelta(m);
                  const qty = Math.abs(delta);
                  const signLabel = delta < 0 ? "نقص" : "زيادة";
                  return `<div class="inv-move-item">
                    <span>${U.d(m.date)} - ${signLabel}: ${Units.format(qty, p)} (${U.q(qty)} كيس)</span>
                    <div class="inv-move-actions">
                      <button class='btn ghost' data-inv-oe='${m.id}'>تعديل</button>
                      <button class='btn danger' data-inv-od='${m.id}'>حذف</button>
                    </div>
                  </div>`;
                })
                .join("") || "<div class='inv-move-empty'>لا توجد عمليات أول مدة مضافة</div>";
            const movesHtml =
              Inv.productFactoryMoves(p.id)
                .slice()
                .sort((a, b) => String(b.date).localeCompare(String(a.date)))
                .slice(0, 5)
                .map(
                  m => `<div class="inv-move-item">
                    <span>${U.d(m.date)} - ${Units.format(m.qty, p)} (${U.q(m.qty)} كيس)</span>
                    <div class="inv-move-actions">
                      <button class='btn ghost' data-inv-me='${m.id}'>تعديل</button>
                      <button class='btn danger' data-inv-md='${m.id}'>حذف</button>
                    </div>
                  </div>`
                )
                .join("") || "<div class='inv-move-empty'>لا توجد عمليات وارد مضافة</div>";
            return `
          <tr>
            <td>${U.e(p.name)}</td>
            <td>
            ...
            <td>
              <button class='btn secondary' data-inv-open='${p.id}'>حفظ أول المدة</button>
              <button class='btn primary' data-inv-in='${p.id}'>إضافة وارد</button>
              <div class="inv-moves-list">${openingMovesHtml}</div>
              <div class="inv-moves-list">${movesHtml}</div>
            </td>
          </tr>`;
          })
  ...
  const B = {
    ...
    dyn() {
      ...
      document.querySelectorAll("[data-inv-oe]").forEach(b => (b.onclick = () => Inv.editOpeningMove(b.dataset.invOe)));
      document.querySelectorAll("[data-inv-od]").forEach(b => (b.onclick = () => Inv.delOpeningMove(b.dataset.invOd)));
      document.querySelectorAll("[data-inv-me]").forEach(b => (b.onclick = () => Inv.editFactoryMove(b.dataset.invMe)));
      document.querySelectorAll("[data-inv-md]").forEach(b => (b.onclick = () => Inv.delFactoryMove(b.dataset.invMd)));
