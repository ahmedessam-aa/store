...
(() => {
  ...
  const Gf = {
    save() {
      ...
      st.d.gifts.push({ id: U.id(), type, productId: pid, qty, unit, bagsQty: bags, value, target, note, date, permitNo: Permit.nextNo() });
  ...
  const Permit = {
    ...
    build(ref) {
      ...
      if (kind === "gift") {
        ...
        const row = {
          product: p ? p.name : "-",
          qty: +g.qty || 0,
          unit: this.unitLabel(g.unit),
          bagsQty: +g.bagsQty || 0,
          price: g.bagsQty > 0 ? (+g.value || 0) / (+g.bagsQty || 1) : 0,
          total: +g.value || 0
        };
        return {
          no: +g.permitNo || g.id,
        ...
        <div class="issue-voucher">
          ...
          <div class="issue-footer">
            <div class="issue-total-card">
              ...
              <strong>${U.m(data.total)} جنيه</strong>
            </div>
          </div>
      ...
      const html = `
        ...
        <head>
          ...
          <style>
            ...
            .issue-footer{margin-top:10px;display:flex;justify-content:flex-end}
    ...
    async pdf(data) {
      ...
      data.items.forEach((it, idx) => {
        const line = `${idx + 1}) ${it.product} | ${U.q(it.qty)} ${it.unit} | ${U.q(it.bagsQty)} كيس | ${U.m(it.total)} جنيه`;
      ...
      y += 4;
      doc.text(`إجمالي الإذن: ${U.m(data.total)} جنيه`, 200, y, { align: "right" });
      y += 20;
        ...
        rows
          .map(g => {
            ...
            return `<tr>
              ...
              <td>${U.e(details)}</td>
              <td>${U.m(g.value)}</td>
              <td><button class='btn secondary' data-permit='gift:${g.id}'>إذن صرف</button></td>
            </tr>`;
          })
